...
|*clusterLabels* +
__optional__|Configure cluster labels to correspond to the binding rules in the `PolicyGenerator` or `PolicyGenTemplate` CRs that you define.
`PolicyGenerator` CRs use the `policyDefaults.placement.labelSelector` field.
`PolicyGenTemplate` CRs use the `bindingRules` field.|object
|*clusterName* +
